...
def estimate_parameters(events,
    ...
    events = [e for e in events
              if e[0] < obs_time]  # e[0] holds time of the event
    ...
    fitted = fit.fit_parameter(estimates=estimations,
                               fun=lambda x, *args: p(x, *args, **p_params),
                               start_values=values,
                               xval=window_middle)
...
def predict(events,
    ...
    lambda_t, total, _ = prediction.predict_nb_retweets(
        events=events_filt,
        obs_time=obs_time,
        pred_time=pred_time,
        p=lambda x: p(x, *params, **p_params),
        p_max=p_max,
        dt=dt,
        kernel=kernel)

    events_time_pred = np.array([ev for ev, _ in events if ev > obs_time])
    pred_original_length = len(events_time_pred)
    ...
    for i, t_cur in enumerate(tp):
        ...
        if t_end > pred_time:
            break
        count_current = estimate.get_event_count(events_time_pred, t_cur,
                                                 t_end)
        pred_count = dt * l_t[(i * win_int):((i + 1) * win_int)].sum()
        err += abs(count_current - pred_count)

    return lambda_t, total, err, pred_original_length
...
def predict_optimized(event_times,
    ...
    lambda_t, total = prediction.predict_nb_retweets_vec(
        event_times=event_times[filt_obs],
        follower=follower[filt_obs],
        obs_time=obs_time,
        pred_time=pred_time,
        p=lambda y: p(y, *params, **p_params),
        kernel=kernel,
        dt=dt,
        p_max=p_max)
...
def training_error(param,
    ...
    if param[2] < 0:  # taum parameter should never be negative
        ...
        add_params = {
            ...
            'phi0': param[1],
            'taum': 1 /
            param[2],  # training algorithm trains 1/taum -> convert back here
            't0': start_time
        }

        params, _, _ = estimate_parameters(events=events,
                                           obs_time=obs_time,
                                           window_size=e_window_size,
                                           window_stride=e_window_stride,
...
def training_error_optimized(param,
                             event_data,
                             obs_time=6,
                             pred_time=168,
                             e_window_size=4,
    ...
    if param[2] < 0:
        ...
        params, _, _ = estimate_parameters_optimized(
        ...
        _, _, err = predict_optimized(event_times=event_times,
                                      follower=follower,
                                      obs_time=obs_time,
                                      pred_time=pred_time,
                                      window=p_window,
...
def training_cross_validation(events_data,
                              iterations,
                              start_values,
                              simplex=None,
                              obs_time=6,
    ...
    for run in range(3):  #iterations
        ...
        filter_in = np.delete(np.arange(len(events_data)),
                              slice(run, None, iterations))
        cur_events = [events_data[i]
                      for i in filter_in]  # filter out the event_data objects
        train_res = training.train_optimized(start_values, cur_events,
                                             obs_time, pred_time,
        ...
        cur_events_check = [
            events_data[i]
            for i in np.arange(run, len(events_data), iterations)
        ]
        for data in cur_events_check:
            err = training_error_optimized(param, data, obs_time, pred_time,
                                           e_window_size, e_window_stride,
